...
______________________________________________________________________
 ...
 worked on and improved at various times over the past 24 years.
...
Version 3.51 December 25, 2024

-added support for a sprite size table for custom overworld sprites. 
 Check the help file for more details.
-added a setting to General Options that can adjust the size of the
 main and custom toolbar in the level editor window, and the toolbar
 in the overworld editor window.
-tweaked the new PC to SNES color conversion from 3.50 a bit. 
 Thanks goes out to Green for prompting me to look into it.
-fixed an issue where Microsoft apparently hasn't updated the version
 5 common controls in the OS to deal with larger mouse pointers that
 end up covering parts of tooltips that are supposed to appear just
 below the mouse cursor.  Also fixed an issue where Microsoft hasn't
 updated the version 5 or version 6 controls to deal with the newer
 mouse pointer size slider setting in Windows 10+ that causes the
 same type of tooltip problem.  Thanks goes out to Mapsking for
 bringing this up.


Version 3.50 September 24, 2024 (24 year Anniversary of Lunar Magic!)

-added "Delete Level from ROM" and "Delete Multiple Levels from ROM"
 menu commands, which can be used to delete levels from the expanded
 area of the ROM and replace them with the "test" level in the
 original ROM area.
-added "-DeleteLevels" to command line functions.
-added LM_NOTIFY_ON_DELETE_LEVEL to button options for custom user
 toolbars.
-made custom user toolbars that use internal LM commands for most
 view menu items automatically use a toggle button so they can now
 show their state.
-added a new "Share Data Between Levels" option that's on by
 default.  When saving a level this will automatically scan existing
 RAT-protected levels to see if they use the same BG, palette, etc as
 the level being saved.  If a match is found, a pointer to the
 existing data can be used which can help save on ROM space.  Note:
 To get the most out of this with a ROM being upgraded from a
 previous version, you may want to use "Export Multiple Levels to
 Files" (modified levels only), "Delete Multiple Levels" (modified
 levels only), then "Import Multiple Levels from Files".
-added the ability to display custom overworld layer 1 Mario path
 images, in case someone edits the path data in the ROM.  Note that
 this only affects display in the editor, not the actual game.  See
 the technical information section in the help file for more info.
-added a new dialog for editing SNES colors as hex in either SNES or
 PC format in the palette editors using middle click or F2.
-changed the gradient generator in the palette editors to use
 somewhat lighter colors at times.
-changed SNES/PC color conversion for display to a newer method that
 results in slightly brighter colors and pure white on SNES being
 pure white on PC.  Also added a option to use the old method
 instead.
-added an option to use the old SNES/PC color conversion method for
 exporting to YY-CHR .pal files, which is recommended to leave on by
 default for better compatibility with older versions of Lunar Magic
 (otherwise old versions could get slightly different SNES colors
 than intended when importing).
-changed the file extension for shared palette files from .pal to
 .smwpal to avoid people confusing them with YY-CHR palette files.
-added the ability to hold Ctrl with the frame group buttons in the
 "Edit ExAnimated Frames" dialogs to move the frame values right or
 left by 1 and effectively insert or delete a frame at the start of
 the animation.
-added the ability to hold Ctrl+Shift with the frame group buttons in
 the "Edit ExAnimated Frames" dialogs to rotate the frame values
 right or left by 1.
-added the ability to hold Alt+Shift to drag entrances in the level
 editor without updating their FG/BG Initial Position settings.
-added a new "Auto-Separate Main/Midway Entrance on Drag" option that
 allows turning off automatically separating the main and midway
 entrance from each other when they're combined and you try to drag
 the main entrance to a new position.
-added a new "Auto-Skip Warnings on Command Line" option that allows
 turning off automatically skipping past warnings that request input
 on the command line.  Note however that turning it off means the
 program will return to using standard GUI message boxes to display
 warnings, as in 3.33 and earlier.
-removed a limitation in the original game where layer 2 for levels
 could not be stored in bank 0xFF.
-added a new ASM hack to expand the pointers to the extra levels in
 Chocolate Island 2 to 24 bits so that just resaving one of levels
 0xCD-0xCF will no longer crash the game if the coin/time mechanism
 has not been disabled (though note that editing the extra levels is
 still not supported).
-gave Lakitu and the Blue Bird a shadow in the overworld editor. 
 Also gave them and custom overworld sprites an up arrow when
 they're displayed above the main map screen.
-the program will no longer allow inserting ExGFX as 4bpp until the
 regular GFX have first been inserted as 4bpp, to avoid problems for
 users that don't read the warning.
-LM will now convert the placeholder berry in GFX file 0x17 from 3bpp
 to 4bpp like the game does, just in case someone wants to use it
 when animation is turned off.  Thanks goes out to Thomas for
 bringing this up.
-fixed a bug from 3.40 where console output on the command line would
 not show a message before opening up a file choosing dialog for the
 restore system, but would instead show it after closing that window. 
 Thanks goes out to AmperSam for reporting this.
-fixed a bug from 1.70 where ROMs last modified with LM 1.10 to 1.20
 that used the second BG page in a custom BG would not upgrade that
 BG to show the correct page in the game even though it looked fine
 in LM.
-made the "Copy Background Image from other Level" button in the BG
 editor no longer show a message about graphics if the graphics have
 been bypassed, as the info was only useful for copying the original
 game's backgrounds.
-made it possible for 3rd party patches to notify LM that the
 overworld is bypassed so LM can display a warning before trying to
 save it.
-made it so that if the VRAM patch option is set to none, LM will
 display non-Direct Map16 objects that cross horizontal subscreen
 boundaries in vertical levels as breaking up, as they did before
 3.00.  Thanks goes out to monkey03297 for bringing this up.
-improved the performance of the standard GFX bypass dialogs for very
 slow systems by using a bit of creative UI trickery.


Version 3.40 September 24, 2023 (23 year Anniversary of Lunar Magic!)

-added support for language packs, which allows displaying Lunar
 Magic's interface in a different language if a translation is
 available (these can be downloaded separately from FuSoYa's site in
 the future).  The language can be set from the "Options", "Language"
 menu.  If no language has yet been set, LM will automatically detect
 and set it the first time it finds the language pack folder.
-added basic table file support for the overworld editor's "Edit Level
 Names", "Edit Message Box Text", and "Edit Boss Sequence Text"
 dialogs that can be used for editing and displaying the text in
 different languages.
-added support for MultiChar tiles in the overworld editor's "Edit
 Level Names" dialog that allows for easier editing and display of
 names that used squished tiles like for "Yellow Switch Palace" and
 "Forest of Illusion" in the original game.  Also added an option to
 turn this off.
-added an option to the restore options that will compress new restore
 points created, which is on by default.  Note that previous versions
 of LM do not support restoring from compressed restore points
 (attempting to make an older version do so doesn't actually harm
 anything, though it may give you a scary message claiming the restore
 file is corrupt).
-added support for importing palettes from Snes9x save state files.
-added "Copy Overworld to Another ROM" menu item to the overworld's
 "File" menu, which is just a shortcut to the command line function
 for the same thing.
-added throw block sprite 53 to the sprite list, updated its
 appearance in LM, and added a tweak by Kevin that makes it usable
 when placed directly in a level.
-fixed a bug from 1.30 where Mario's fireballs would not trigger
 custom block ASM for blocks on layer 2 in layer 2 levels.  The fix
 will be installed when the Map16 is saved.  Thanks goes out to Kevin
 for reporting this.
-fixed a bug from 1.40 where the "Future Layer 1 Tiles" view option
 in the overworld editor did not correctly display a tile as
 "revealed" in cases where a layer 1 tile could be revealed by more
 than one event and the first event was not passed while the second
 one with a higher event number was.  Thanks goes out to
 MarioFanGamer for reporting this.
-fixed a bug from 1.40 where opening mwl files created before 1.40
 could give you incorrect settings for the main/midway entrance.
-fixed a bug from 3.20 where if the DirectX option was on and
 animation was off and you deleted objects/sprites in the level editor
 that were covered by a tooltip, the area under the tooltip might
 not be refreshed until the screen was refreshed.
-fixed a bug from 3.30 where the "Auto-Enable custom palette on edit"
 setting didn't enable using a custom palette soon enough before
 doing a gradient, which could result in only a partial gradient being
 done.
-made it possible to do vertical gradients in the palette editors
 using Alt+Shift+Right-Click.
-the "Conditional Direct Map16" command in the edit menu will now
 display the existing settings of the selected object(s), when
 possible.
-changed how the current directory and relative paths are handled for
 command line functions, which should make it more consistent. 
 Thanks goes out to Ragey for bringing this up.
-when using command line functions, standard GUI message boxes will no
 longer be displayed and will instead output their message on the
 command line.  If they request input, ok/yes will automatically be
 used.
-changed the "-ImportMultLevels" command line function a bit so that
 it will return a fail code if 1 or more levels fail to insert.
-added "-ChangeCompression" to command line functions.  Make sure to
 read the warning in the help file before using it.
-added a warning message for when an ExAnimation destination is set
 to a disabled slot or you've assigned the same one shot trigger
 number to more than one slot.  Also added a "More ExAnimation Checks"
 option to "General Options" to disable the warnings.
-moved the spacebar shortcut for "Preview Layer 3 in Level" in the
 Overworld Editor window to `.
-made it possible to hold down spacebar when using the mouse scroll
 wheel to scroll at x3 speed in several editor windows.
-made it possible to use Ctrl+Left-Click in the Overworld Editor
 window to select/deselect sprites in sprite editing mode.
-added "Select All" with Ctrl+A to the Background Editor window and
 some modes of the Overworld Editor window.
-added a tile grid to the Background Editor and the 8x8 Tile Editor
 windows (F8 key).
-made the tile grid in the main level editor and the Background Editor
 somewhat translucent when the "Translucent Text and Outlines" option
 is on.
-renamed the "Variable" and "Normal" scroll speeds to "Medium", as
 it's more consistent with the others and makes more sense.
-added "Medium 3", "Medium 4", "Slow 2", "Auto-Scroll Fast 3" and
 "Auto-Scroll Fast 4" scroll settings to the Layer 3 Settings dialog.
-added "Fast" and several "Auto-Scroll" settings for Layer 2.
-added a setting that lets you use separate settings for vertical and
 horizontal layer 2 scroll instead of using the preset paired settings
 of the original game.
-moved the "Auto-Set Number of Screens" option from "General Options"
 to the "Change Properties in header" dialog, as this is now a
 per-level setting.
-removed the "Auto-Set Number of Screens" setting from the "Import
 Multiple Levels from Files" dialog and the "-ImportMultLevels"
 command line function as it's now been made obsolete by the
 per-level setting.
-tweaked a few of the original levels so the "Auto-Set Number of
 Screens" option doesn't have to be turned off for them.
-made it possible to type the number of screens for the level in the
 drop down list box in the "Change Properties in header" dialog.
-moved the "Vertical Level Positioning" and the "Disable No Yoshi
 Level Intro" options from the "Change Other Properties" dialog to
 the "Change Properties in header" dialog.
-renamed the "Change Other Properties" dialog to "Change Layer 1/2
 Properties".
-moved the layer 1 scroll setting from "Change Properties in header"
 dialog to the "Change Layer 1/2 Properties" dialog.
-saving the credits will now save the full viewable area in the
 editor, which can be used in the game with some tweaking.  It
 should also now use less space most of the time.
